	...
	if (IsLocalPlayerController())
	{
		if (SVirtualJoystick::ShouldDisplayTouchInterface())
		{
			...
			if (MobileControlsWidget)
			{
				...
				UE_LOG(Logrcsim, Error, TEXT("Could not spawn mobile controls widget."));
		...
		if (UIWidget)
		{
			...
			UE_LOG(Logrcsim, Error, TEXT("Could not spawn Time Trial UI widget."));

		}
		

		// spawn the UI widget and add it to the viewport
		VehicleUI = CreateWidget<UrcsimUI>(this, VehicleUIClass);
		...
		if (VehicleUI)
		{
			...
			UE_LOG(Logrcsim, Error, TEXT("Could not spawn vehicle UI widget."));
...
void ATimeTrialPlayerController::OnPossess(APawn* InPawn)
{
	...
	VehiclePawn = CastChecked<ArcsimPawn>(InPawn);
...
void ATimeTrialPlayerController::OnPawnDestroyed(AActor* DestroyedPawn)
{
	...
	if (ActorList.Num() > 0)
	{
		...
		if (ArcsimPawn* RespawnedVehicle = GetWorld()->SpawnActor<ArcsimPawn>(VehiclePawnClass, SpawnTransform))
		{
			// possess the vehicle
